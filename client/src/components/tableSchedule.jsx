...
function cell_color(text) {
  // console.log(text);
    ...
    return { 
      props: {
        style: { background: col_row, borderRadius: "10px", marginBottom: "5px"},
...
const TableSchedule = ({resSchedule}) => {
  ...
  const [dataFilterAM, setDataFilterAM] = useState(data.filter((row) => row.Block === 'AM'))
  const [dataFilterPM, setDataFilterPM] = useState(data.filter((row) => row.Block === 'PM'))
  ...
  const handleBlock = (value) => {
    ...
    return (
      <>
        ...
        <Table className='ant-table-layout-fixed table' style={{marginBottom: 5}} bordered pagination={{ pageSize: 50 }} scroll={{x: 'max-content'}}size='small' columns={block === 'AM' ? AM: PM} sticky dataSource={block === 'AM' ? dataFilterAM: dataFilterPM} onChange={handleChange} rowKey={record => record.uid}/>
      </>
    );
}
