...
* @setReschedule: this setter sends the response schedule data from the back-end to be used to
* build the datatable.
* @setView: is used to redirect automatically from the generate meeting section to the meeting table view
*/
const UploadFile = ({setResSchedule, setView}) => {
  ...
  * the user to the scheduling table */
  ...
  /* This effect redirects from this component to the table schedule view and cleans this component*/
  useEffect(() => {
    if (fetched === true) {
  ...
  const handleSubmit = () => {
    if (jsonData !== null) {
      if (window.confirm("A new meeting schedule is going to be generated\nand the actual schedule will be erased\nDo you want to continue?")) {
        ...
        setLoading(1);
        fetch(
          'http://localhost:3033/api/schedule', /* Route to send the CSV file to generate the schedule*/
          {
              method: 'POST',
              headers: {
          ...
          .then((result) => {
              ...
              setIsReset(true); 
              /* with this we call the use effect to redirect to other component and clean the actual component */
              setFetched(true);
          })
          .catch((error) => {
              setIsReset(true);
  ...
  /* Removes the file from the front-end to allow the upload of another file or when an error ocurred */
